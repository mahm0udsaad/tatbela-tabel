...
export default async function BlendsPage({ searchParams }: { searchParams: Promise<{ search?: string }> }) {
  ...
  const blendsCategory = categories.find((cat) => cat.slug === "blends")
        ...
        <div className="max-w-7xl mx-auto px-4 py-10">
          <div className="flex flex-col md:flex-row items-start justify-between gap-6">
            <div>
              ...
              <h1 className="text-4xl font-bold text-[#2B2520] mb-4">{blendsCategory?.name_ar ?? "الخلطات"}</h1>
              <p className="text-lg text-[#8B6F47] max-w-2xl">
                اكتشف مجموعتنا المميزة من الخلطات المصرية والأعشاب الأصيلة، مصنوعة بعناية من أفضل التوابل الطبيعية لتضيف نكهة فريدة لأطباقك.
              </p>
            </div>
