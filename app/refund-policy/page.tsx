...
import { DEFAULT_REFUND_POLICY } from "@/lib/site-content/defaults"
import { policyPagePayloadSchema } from "@/lib/site-content/schemas"

export const metadata: Metadata = {
  title: "سياسة الاسترجاع والاسترداد | تتبيلة & تابل",
  description: "سياسة الاسترجاع والاستبدال للمنتجات المشتراة من تتبيلة وتابل.",
}
...
export default async function RefundPolicyPage() {
  ...
  if (error) {
    console.error("Failed to load refund policy settings", error)
  }
  ...
  const payload = parsed.success ? parsed.data : DEFAULT_REFUND_POLICY
      ...
      <main className="min-h-screen">
        <section className="py-24 px-4">
          <div className="max-w-2xl mx-auto text-center">
            <h1 className="text-4xl font-bold text-[#2B2520] mb-4">سياسة الاسترجاع والاسترداد</h1>
            ...
            <div className="space-y-6 text-[#5C5347] leading-8 text-right">
              ...
              {payload.footer && (
                <div className="pt-6 border-t border-[#E8E2D1] mt-8">
                  <p className="font-bold">هل لديك استفسار؟</p>
                  <p>{payload.footer}</p>
                </div>
              )}
            </div>
