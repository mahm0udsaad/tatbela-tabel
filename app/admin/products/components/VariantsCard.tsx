...
export function VariantsCard({
  ...
  const isPerKilo = selectedProduct?.pricing_mode === "per_kilo"
  const basePrice = selectedProduct?.price_per_kilo
    ...
    <div className="bg-white rounded-2xl shadow p-6 space-y-4">
      ...
      {isPerKilo && (
        <div className="text-xs text-[#8B6F47] space-y-1">
          ...
          {basePrice ? <p>سعر الكيلو الحالي: {formatCurrency(basePrice)}</p> : <p className="text-red-600">يرجى ضبط سعر الكيلو في بيانات المنتج.</p>}
        </div>
      )}
