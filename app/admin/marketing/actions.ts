...
export async function fetchPhoneContactsAction(input: {
  ...
  for (const row of orderRes.data ?? []) {
    ...
    if (phoneMap.has(normalized)) {
      ...
      phoneMap.set(normalized, { phone: normalized, name, source: "order" })
    }
  }
  ...
  for (const row of addressRes.data ?? []) {
    ...
    if (phoneMap.has(normalized)) {
      const existing = phoneMap.get(normalized)!
      if (!existing.name && name) existing.name = name
      if (existing.source === "order") existing.source = "both"
    } else {
      phoneMap.set(normalized, { phone: normalized, name, source: "address" })
    }
  }
