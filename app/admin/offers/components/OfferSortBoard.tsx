...
type OfferSortBoardProps = {
  initialOffers: SortableOffer[]
  title?: string
  description?: string
...
function SortableOfferRow({ offer, index }: { offer: SortableOffer; index: number }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({ id: offer.id })

  const primaryImage = offer.offer_images?.find((img) => img.is_primary) ?? offer.offer_images?.[0]
  const isOutOfStock =
    (offer.stock ?? 0) <= 0 &&
  ...
  return (
    ...
    >
      ...
      <div className="p-4 flex flex-col flex-1">
        ...
        <h3 className="text-lg font-bold text-[#2B2520] mb-1 line-clamp-2">{offer.name_ar}</h3>
        <p className="text-sm text-[#8B6F47] line-clamp-2 mb-3">{offer.description_ar}</p>
          ...
          <div className="flex">
            {Array.from({ length: 5 }).map((_, starIndex) => (
              <Star
                ...
                size={14}
                className={
                  starIndex < Math.round(offer.rating || 0) ? "text-[#E8A835] fill-[#E8A835]" : "text-gray-300"
                }
              />
            ))}
          </div>
        ...
        <div className="flex items-baseline gap-2 mt-auto">
          <span className="text-2xl font-bold text-[#C41E3A]">{formatCurrency(offer.price)}</span>
          {offer.original_price && offer.original_price > offer.price && (
...
export function OfferSortBoard({
  initialOffers,
  title,
  description,
  emptyStateLabel,
  ...
  const [items, setItems] = useState<SortableOffer[]>([])
  ...
  const sortedInitialOffers = useMemo(
    () =>
      [...initialOffers].sort(
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const result = await updateOffersSortOrderAction(updates)
    setIsSaving(false)
    ...
    toast({
      title: "تم الحفظ",
      description: "تم تحديث ترتيب العروض",
    })
  }

  const headingTitle = title ?? "ترتيب العروض"
  const headingDescription = description ?? "اسحب وأفلت لتحديد ترتيب ظهور العروض في صفحة /offers."
  const noItemsLabel = emptyStateLabel ?? "لا توجد عروض لعرضها حالياً."
  const badgeText = badgeLabel ?? "عدد العروض:"
          ...
          <SortableContext items={items} strategy={rectSortingStrategy}>
            <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-6">
              {items.map((offer, index) => (
                <SortableOfferRow key={offer.id} offer={offer} index={index} />
              ))}
            </div>
          </SortableContext>
        </DndContext>
