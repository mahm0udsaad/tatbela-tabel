...
import type { CategoryOption, ProductFormState } from "@/app/admin/products/types"

type B2BProductDetailsCardProps = {
...
export function B2BProductDetailsCard({
  productForm,
  onFieldChange,
  categoryOptions,
  onAddCategory,
  ...
  useEffect(() => {
    if (!productForm.is_b2b) {
      onFieldChange("is_b2b", true)
    }
  }, [productForm.is_b2b, onFieldChange])
      ...
      <div className="p-6 space-y-6">
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* B2B Product - Always enabled, display only */}
            <div className="flex items-start gap-3 rounded-xl border-2 border-[#E8A835] bg-[#FFF8ED] p-4">
              <div className="mt-0.5">
                <input
                  type="checkbox"
                  checked={true}
                  disabled
                  className="accent-[#E8A835] w-5 h-5 cursor-not-allowed opacity-60"
                />
              </div>
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  <Building2 size={16} className="text-[#E8A835]" />
                  <p className="text-sm font-semibold text-[#2B2520]">منتج الجملة (B2B)</p>
                </div>
                <p className="text-xs text-[#8B6F47] leading-relaxed">
                  جميع المنتجات في هذه الصفحة هي منتجات جملة (B2B)
                </p>
              </div>
            </div>
